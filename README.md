...
* 2.3.6 **接收HTTP响应消息**<br/>
  ...
  判断数据是否有遗漏，往往需要通过**上一次的ACK号和接收到的数据的序号**，比如上一次ACK号为1460字节，即已经接收到了序号为1460的字节数据，接下来如果收到的数据的序号为1461，那么说明数据没有遗漏，否则就说明有数据遗漏了；<br/>

### 2.4从服务器断开并删除套接字
* 2.4.1 **数据发送完毕后断开连接**<br/>
  断开连接的时候，**可以服务器先发起断开连接的请求，也可以由客户端先发起断开连接的请求**.<br/>
  在http1.0中，当客户端发送请求，**到服务器响应请求结束后**，服务器已经没有数据要发送给客户端了，所以服务器就会发起断开连接的请求，服务器就会**调用Socket库中的close组件来结束数据收发的操作，生成一个FIN为1的TCP包，然后委托IP模块发送给客户端，一段时间后客户端返回ACK确认号即同意服务器断开连接**，当客户端接收到了服务器发送过来的所有响应数据也需要断开和服务器的连接，同样客户端会**调用Socket库中的close组件来结束数据收发的操作，生成一个FIN为1的TCP包，然后委托IP模块发送给服务器，一段时间后服务器返回ACK确认号即同意客户端断开连接**<br/>
  而在http1.1中，服务器返回响应之后，客户端还可能会发起下一个请求，**如果客户端接下来没有请求要发送了**，那么**客户端就会先向服务器发起断开连接的请求**<br/>

* 2.4.2 **删除套接字**<br/>
  和服务器断开连接之后，**套接字不会立即删除，而是要等待一段时间(通常为几分钟)之后再删除**, 之所以要等待一段时间，是**为了防止一些误操作**，比如，当服务器向客户端发送FIN，客户端需要对这个FIN进行确认，返回一个ACK，但是如果服务器没有收到这个ACK，那么服务器就需要重新发送FIN，如果这个时候客户端的套接字已经删除了，那么如果此时另一个应用程序也正好创建了一个和刚才端口号相同的套接字，那么这个FIN就会被这个新创建的套接字接收到而进行断开连接的操作，造成一些错误<br/>
